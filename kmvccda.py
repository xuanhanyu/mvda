...
def within_class_vars(mv_Xs, y, alpha=1, beta=1):
    num_views = len(mv_Xs)
    y_unique = torch.unique(torch.tensor(y))
    ...
    for j in range(num_views):
        ...
        for r in range(num_views):
            if j == r:
                s_jr = alpha * (I - W) + beta * 2 / num_views * (I - D)
            else:
                s_jr = alpha * -W - beta * 2 / num_views * D
...
def between_class_vars(mv_Xs, y, theta=1, gamma=1):
    num_views = len(mv_Xs)
    dims = [len(ws[0]) for ws in mv_Xs]
    y_unique = torch.unique(torch.tensor(y))
    ecs = [torch.tensor([1 if _ == ci else 0 for _ in y], dtype=torch.float) for ci in y_unique]
    ...
    I = torch.eye(len(y)) * num_views
    E = torch.zeros(len(y), len(y))
    for ca in y_unique:
        for cb in y_unique:
            if ca != cb:
                E += ecs[ca].unsqueeze(0).t() @ ecs[cb].unsqueeze(0)
    ...
    for j in range(num_views):
        ...
        for r in range(num_views):
            if j == r:
                s_jr = theta * (W - B) + 2 * gamma * (I - E)
            else:
                s_jr = theta * (W - B) + 2 * gamma * -E
            s_jr = mv_Xs[j].t() @ s_jr @ mv_Xs[r]
            S_rows.append(s_jr)
        S_cols.append(torch.cat(S_rows, dim=1))
